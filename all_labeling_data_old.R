...
d.good <- d.good %>% filter(Compound != "aKG") %>% 
  mutate(Compound = ifelse(Compound == "Glu", "aKG", Compound))

d.good$Compound %>% unique()




# fasted and fed state each as a separate file
d.13C.fasted <- d.good %>% filter(State == "fasted")
d.13C.refed  <- d.good %>% filter(State == "refed")
...
#             sd = sd(labeling)) %>% 
#   # filter(C_Label != 0) %>% 
#   ggplot(aes(x = 1, y = lab, fill = as.character(C_Label))) +
#   geom_col(position = "stack", color = "black") +
